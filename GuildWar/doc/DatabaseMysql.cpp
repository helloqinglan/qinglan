...
bool DatabaseMysql::initialize(const char *infoString)
{
	...
	if (!mysqlInit)
	{
        ACE_ERROR ((GAME_ERROR ACE_TEXT("DatabaseMysql::Initialize 初始化mysql失败.\n")));
        return false;
	}
	...
	if (tokens.size() != 5)
	{
		ACE_ERROR ((GAME_ERROR 
			ACE_TEXT("DatabaseMysql::Initialize 数据库配置项错误, 格式必须为hostname;port;username;password;database.\n")));
		return false;
	}
	...
	if (!m_mysqlHandle)
	{
		ACE_ERROR ((GAME_ERROR ACE_TEXT("DatabaseMysql::Initialize 连接到数据库%s失败, 原因%s.\n"), 
	...
	ACE_DEBUG ((GAME_DEBUG ACE_TEXT("DatabaseMysql::Initialize 连接到数据库%s成功.\n"), tokens[0].c_str()));
...
QueryResult* DatabaseMysql::pquery(const char *format, ...)
{
    ...
    if (res == -1)
    {
		ACE_ERROR ((GAME_ERROR ACE_TEXT("DatabaseMysql::pquery 无效的格式化字符串: %s.\n"), format));
        return 0;
    }
...
QueryResult* DatabaseMysql::query(const char* sql)
{
    ...
    {
		ACE_ERROR ((GAME_ERROR ACE_TEXT("DatabaseMysql::query 查询失败, 原因: %s.\n"), mysql_error(m_mysqlHandle)));
		return 0;
    }
...
bool DatabaseMysql::execute(const char* sql)
{
    ...
    if(mysql_query(m_mysqlHandle, sql))
	{
		ACE_ERROR ((GAME_ERROR ACE_TEXT("DatabaseMysql::execute 执行失败, 原因: %s.\n"), mysql_error(m_mysqlHandle)));
		return false;
	}
...
bool DatabaseMysql::pexecute(const char* format, ...)
{
	...
	if (res == -1)
	{
		ACE_ERROR ((GAME_ERROR ACE_TEXT("DatabaseMysql::pexecute 无效的格式化字符串: %s.\n"), format));
		return false;
	}
